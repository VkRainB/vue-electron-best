  ...
  <el-container class="sidebar-layout w-screen h-screen overflow-hidden">
    ...
    >
      <div class="sidebar-content h-full flex flex-col">
        <!-- 侧边栏头部 -->
        <div class="sidebar-header p-4 border-b border-gray-100 bg-gradient-to-r from-indigo-500 to-purple-600">
          <div v-if="!sidebarCollapsed" class="flex items-center gap-3">
            <div class="w-8 h-8 bg-white/20 rounded-lg flex items-center justify-center backdrop-blur-sm">
              <Icon name="el-icon-House" size="1.5em" class="text-5 flex-shrink-0" />
            </div>
        ...
        <div class="sidebar-menu flex-1 p-2 overflow-y-auto custom-scrollbar">
          <nav class="space-y-1">
            ...
            >
              <el-icon class="text-5 flex-shrink-0">
                <Icon name="el-icon-DataBoard" size="1.5em" class="text-5 flex-shrink-0" />
              </el-icon>
            ...
            <!-- 分隔线 -->
            <div v-if="!sidebarCollapsed" class="my-2 border-t border-gray-200" />
        ...
        <div class="sidebar-footer p-2 border-t border-gray-100">
          ...
          >
            <el-icon class="text-5 flex-shrink-0">
              <Icon
                :name="sidebarCollapsed ? 'el-icon-Expand' : 'el-icon-Fold'"
                size="1.5em"
    ...
    <el-container class="flex-1 overflow-hidden">
      <!-- 顶部工具栏 -->
      <el-header class="header-toolbar h-16 bg-white border-b border-gray-200 shadow-sm p-0 flex items-center">
        <div class="w-full h-full flex items-center justify-between px-6">
            ...
            <el-breadcrumb separator="/" class="text-sm">
              ...
              >
                <el-icon v-if="item.icon" class="mr-1">
                  <Icon :name="`el-icon-${item.icon}`" size="1em" class="mr-1" />
                </el-icon>
                {{ item.title }}
              </el-breadcrumb-item>
            </el-breadcrumb>
          ...
          <div class="flex items-center gap-2">
            ...
            <el-dropdown @command="handleUserCommand">
              ...
              >
                <Icon name="i-weui-user-outlined" size="20px" color="currentColor" tooltip="用户菜单" />
                <span class="text-sm font-medium">用户</span>
                <el-icon class="text-3">
                  <Icon name="el-icon-ArrowDown" size="1.2em" class="text-3" />
                </el-icon>
              </button>
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item command="profile">
                    <Icon name="i-weui-user-outlined" size="16px" color="currentColor" class="mr-2" />
                    个人资料
                  </el-dropdown-item>
                  <el-dropdown-item command="settings">
                    <Icon name="i-weui-setting-outlined" size="16px" color="currentColor" class="mr-2" />
                    设置
                  </el-dropdown-item>
